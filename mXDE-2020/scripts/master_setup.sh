...
post_device() {
 echo post device
 ./post_resource.sh Device ../templates/fhir_device.xml ../logs/fhir_device.xml.txt
 if [ $? != 0 ] ; then
  echo "./post_resource.sh Device ../templates/fhir_device.xml ../logs/fhir_device.xml.txt" failed
  exit 1
 fi
}
...
post_location() {
 echo post location
 ./post_resource.sh Location ../templates/fhir_location.xml ../logs/fhir_location.xml.txt
 if [ $? != 0 ] ; then
  echo "./post_resource.sh Location ../templates/fhir_location.xml ../logs/fhir_location.xml.txt" failed
  exit 1
 fi
}
...
post_organization() {
 ...
 ./post_resource.sh Organization ../templates/fhir_organization.xml ../logs/fhir_organization.xml.txt
 if [ $? != 0 ] ; then
  echo "./post_resource.sh Organization ../templates/fhir_organization.xml ../logs/fhir_organization.xml.txt" failed
  exit 1
 fi
}
...
# After the split, .aa contains HTTP junk and .ab contains XML
...
for patient_resource in Farnsworth-NA2020 ; do
 post_patient ../fhir_patients/$patient_resource.xml ../logs/$patient_resource.xml.txt
 map_patient $MAP
done
